...
function handlerNext() {
  location.replace('NB0001_02.html');
}

// 상단
var topArea = Paper.g();

voca01.use().transform('t90, 80').click(playAud01).appendTo(topArea).attr({
  'cursor': 'pointer'
});

var t1 = topArea.text(180, 300, ['がくせい', '학생']).click(playAud01).attr({
  'font-size': 40,
  'text-anchor': 'middle',
  'cursor': 'pointer'
});
...
function handler() {
  ...
  // bottomArea.text(180, 605, 'わたしは がくせいです。').attr({
  //   'font-size': 25,
  //   'text-anchor': 'middle'
  // });

  var t3 = bottomArea.text(60, 540, ['わたし', 'は', 'がくせい', 'です。']).attr({
    'font-size': 25,
    'fill': 'gray'
  });
