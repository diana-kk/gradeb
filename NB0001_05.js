...
var voca05 = Paper.image('img/voca05.png', 0, 0, 180, 150).toDefs();
...
function handlerNext() {
  location.replace('NB0001_06.html');
}

// 상단
var topArea = Paper.g();

voca05.use().transform('t90, 80').click(playAud01).appendTo(topArea).attr({
  'cursor': 'pointer'
});

var t1 = topArea.text(180, 300, ['うんてんしゅ', '운전기사']).click(playAud01).attr({
  'font-size': 40,
  'text-anchor': 'middle',
  'cursor': 'pointer'
});
...
bottomArea.text(180, 462, '나는 운전기사입니다.').attr({
  'font-size': 18,
  'text-anchor': 'middle'
});
...
function handler() {
  ...
  // bottomArea.text(180, 605, 'わたしは うんてんしゅです。').attr({
  //   'font-size': 25,
  //   'text-anchor': 'middle'
  // });

  var t3 = bottomArea.text(43, 540, ['わたし', 'は', 'うんてんしゅ', 'です。']).attr({
    'font-size': 25,
    'fill': 'gray'
  });
