...
var voca04 = Paper.image('img/voca04.png', 0, 0, 180, 150).toDefs();
...
function handlerNext() {
  location.replace('NB0001_05.html');
}

// 상단
var topArea = Paper.g();

voca04.use().transform('t90, 80').click(playAud01).appendTo(topArea).attr({
  'cursor': 'pointer'
});

var t1 = topArea.text(180, 300, ['アナウンサー', '아나운서']).click(playAud01).attr({
  'font-size': 40,
  'text-anchor': 'middle',
  'cursor': 'pointer'
});
...
bottomArea.text(180, 462, '나는 아나운서입니다.').attr({
  'font-size': 18,
  'text-anchor': 'middle'
});
...
function handler() {
  ...
  // bottomArea.text(180, 605, 'わたしは アナウンサーです。').attr({
  //   'font-size': 25,
  //   'text-anchor': 'middle'
  // });

  var t3 = bottomArea.text(40, 540, ['わたし', 'は', 'アナウンサー', 'です。']).attr({
    'font-size': 25,
    'fill': 'gray'
  });
