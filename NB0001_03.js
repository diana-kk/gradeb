...
function handlerNext() {
  location.replace('NB0001_04.html');
}

// 상단
var topArea = Paper.g();

voca03.use().transform('t90, 80').click(playAud01).appendTo(topArea).attr({
  'cursor': 'pointer'
});

var t1 = topArea.text(180, 300, ['かんごふ', '간호사']).click(playAud01).attr({
  'font-size': 40,
  'text-anchor': 'middle',
  'cursor': 'pointer'
});
...
bottomArea.text(180, 462, '나는 간호사입니다.').attr({
  'font-size': 18,
  'text-anchor': 'middle'
});
...
function handler() {
  ...
  // bottomArea.text(180, 605, 'わたしは かんごふです。').attr({
  //   'font-size': 25,
  //   'text-anchor': 'middle'
  // });

  var t3 = bottomArea.text(60, 540, ['わたし', 'は', 'かんごふ', 'です。']).attr({
    'font-size': 25,
    'fill': 'gray'
  });
